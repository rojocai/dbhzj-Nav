...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| **1.0.9** | **2026-07-12** | **🌐 完整中英文切换 + 🐛 大量 BUG 修复** |
| 1.0.8 | 2026-07-12 | 🐛 气泡修复+Logo增大+字数统计 |
| 1.0.7 | 2026-07-07 | 🖼️ Logo自动抓取+上传缩放+国旗图标 |
| 1.0.6 | 2026-07-06 | 💾 备份与还原 |
| 1.0.5 | 2026-07-05 | 👁️ 访客统计+IP定位 |
| 1.0.4 | 2026-07-04 | 📱 手机端适配 |
| 1.0.3 | 2026-07-03 | 📱 手机端适配 |
| 1.0.2 | 2026-07-02 | 🐳 Docker支持 |
| 1.0.1 | 2026-07-01 | 初始发布 |
...
- 🗑️ **移除底部多余链接**：删除首页底部链接
- 🧹 **代码清理**：`users.js
## 🆕 v1.0.9 更新内容（相对于 v1.0.8）

### 🌐 新增功能
- **完整中英文切换** — 后台右上角 EN/中文 切换，全部界面文字自动翻译
- **联系方式编辑** — 电话/邮箱/微信/QQ/Telegram/Discord/Twitter/GitHub/地址（后台可配）
- **后台页面内容编辑** — 关于我们/隐私政策/服务条款 后台编辑器
- **版权/Powered by 自定义** — 后台可修改页脚版权文字和 Powered by 信息
- **208 个 Emoji 分类选择器** — 13 个折叠大类，分类标题 EN 模式自动翻译

### 🐛 BUG 修复
- 气泡被下一个分类遮挡 → 改为浮动层 z-index 99999
- 添加/编辑站点弹窗布局优化 — 序号 80px + 图标URL 同行，说明单独一行
- 分类名称 EN 模式自动切换（配置 enName 时）
- 选择文件按钮 → 隐藏原生 input，替换为自定义按钮（支持英文显示）
- 周/日期选择器 EN 显示 — Mon/Tue/1st/2nd/3rd...
- 弹窗按钮确定→OK 即时替换（无需等待翻译）
- 文字样式面板点击空白处自动收起
- JS 语法多处修复，服务稳定性提升
- 图标上传按钮缺失 `<button>` 标签修复

### 💄 界面优化
- emoji 选择器点击空白区域自动关闭
- 上传按钮统一：📤 图标上传 / 📤 Upload Icon
- 所有输入框 placeholder / title 属性支持中英文切换on` 从 git 跟踪中移除，加入 `.gitignore`
