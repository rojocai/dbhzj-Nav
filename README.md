...
- 🗑️ **移除底部多余链接**：删除首页底部 blog.dbhzj.top 链接
...
### v1.0.7 (2026-07-07)
- 🗺️ **访客位置显示优化**：位置信息前添加国旗图标（支持 30+ 国家/地区）
- ⚡ **GeoIP 查询恢复**：恢复 IP 地理位置查询（1 秒超时，不影响页面加载）
- 🐛 **备份功能修复**：修复 cookie HttpOnly 导致备份按钮无响应的 bug
- 🔧 **保存按钮同步**：保存按钮同步写入自动备份设置
- 🗑️ **移除底部链接**：删除首页底部 blog.dbhzj.top 链接
